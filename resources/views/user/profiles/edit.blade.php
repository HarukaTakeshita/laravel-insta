        ...
        <div class="col-8">
            <form action="{{ route('profile.update') }}" method="post" class="shadow bg-white rounded-3 p-5" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col align-self-end">
                        ...
                        @error('avatar')
                            <p class="m-0 text-danger small">{{ $small }}</p>
                        @enderror
                    </div>
                </div>
                ...
                <textarea name="introduction" id="introduction" rows="3" class="form-control">{{ Auth::user()->introduction }}</textarea>
            ...
            <form action="{{ route('profile.updatePassword') }}" method="post" class="shadow bg-white rounded-3 p-5 mt-5">
                ...
                @if(session('password_success'))
                    <p class="text-success small">{{ session('password_success') }}</p>
                @endif
